...
from src.aspire.image import Image, normalize_bg
from src.aspire.image.xform import (
    Downsample,
    FilterXform,
    IndexedXform,
    LambdaXform,
...
from src.aspire.operators import (
    IdentityFilter,
    LambdaFilter,
    MultiplicativeFilter,
    PowerFilter,
)
from src.aspire.storage import MrcStats, StarFile
from src.aspire.utils import Rotation, grid_2d
...
class ImageSource:
    ...
    def cache(self, batch_size=np.inf, verbose=False):
        cached_im = Image(np.zeros((self.n,self.L,self.L)),dtype=np.float32)
        if verbose:
            title = datetime.now().strftime("%Y-%m-%d %H:%M:%S,000 INFO     Caching images:")
            progress_shadybar = progress.bar.Bar(title, max=self.n)
            progress_shadybar.next(0)
        for i in range(0, self.n, batch_size):
            cached_im[i:min(self.n,i+batch_size)] = self.images(start=i, num=batch_size).data
            if verbose:
                progress_shadybar.next(cached_im[i:min(self.n,i+batch_size)].shape[0])
        if verbose:
            print(" ")
        self._cached_im = cached_im
