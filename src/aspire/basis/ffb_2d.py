...
from src.aspire.basis import FBBasis2D
from src.aspire.basis.basis_utils import lgwt
from src.aspire.image import Image
from src.aspire.nufft import anufft, nufft
from src.aspire.numeric import fft, xp
from src.aspire.utils import complex_type
from src.aspire.utils.matlab_compat import m_reshape
...
def my_nufft(sig_f, fourier_pts):
    batch_size = 8192
    num_batches = int(np.ceil(sig_f.shape[0] / batch_size))
    res = np.zeros((sig_f.shape[0],sig_f.shape[1]*sig_f.shape[2]*2),dtype=np.complex64)
    for i in range(num_batches):
        res[i*batch_size:(i+1)*batch_size,:] = nufft(sig_f[i*batch_size:(i+1)*batch_size,:,:], fourier_pts)
    return res

def my_anufft(sig_f, fourier_pts, sz, real=False):
    batch_size = 8192
    num_batches = int(np.ceil(sig_f.shape[0] / batch_size))
    res = np.zeros((sig_f.shape[0],sig_f.shape[1]*sig_f.shape[2]*2),dtype=np.complex64)
    for i in range(num_batches):
        res[i*batch_size:(i+1)*batch_size,:] = anufft(sig_f[i*batch_size:(i+1)*batch_size,:,:], fourier_pts, sz, real=real)
    return res
